...
@section('page-title', 'Alert Management')
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($alerts as $alert)
                    <tr class="hover:bg-blue-50/50 transition duration-150 ease-in-out group {{ !$alert->is_read ? 'bg-blue-50/30' : '' }}">
                        
                        {{-- ID --}}
                        <td class="p-4 text-gray-400 font-mono text-sm">#{{ $alert->id }}</td>
                        
                        {{-- Title & Message --}}
                        <td class="p-4">
                            <div class="font-bold {{ $alert->is_read ? 'text-gray-600' : 'text-gray-900' }}">
                                {{ $alert->title }}
                            </div>
                            <div class="text-xs text-gray-500 mt-1 max-w-xs truncate" title="{{ $alert->message }}">
                        ...
                        <td class="p-4">
                            ...
                            @endphp
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $typeClasses }}">
                                {{ ucfirst($alert->type) }}
                            </span>
                        </td>
                        ...
                        <td class="p-4 text-right">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                {{-- Mark as Read Button --}}
                                @if(!$alert->is_read)
                                    <form action="{{ route('admin.alerts.read', $alert->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
